...
setwd("/Users/kuowenhsi/Library/CloudStorage/OneDrive-WashingtonUniversityinSt.Louis/MOBOT/MOBOT_Boltonia/data/fastsimcoal2/downstream/bestrun")

opt <- list()
opt$prefix <- "downstream"
opt$list <- "A,B,C,D,E,F"
...
plotModelBestParams <- function (prefix="",logNe=FALSE) {
  ...
  if(migMatrixN>0){
    ...
    maxMig=max(as.numeric(unlist(mig)))
    ...
    #       Arrows(x0=sourcePop,x1=sinkPop,y0=add,y1=add,lwd=3,
    #              arr.width=0.1,arr.adj=1,arr.type="simple",
    #              col=rgb(1,1-standStrength,0,maxColorValue=1))
    #       text(x=sourcePop,y=-0.2+add,cex=0.8,
    #            labels=paste(format(edge,scientific=TRUE,digits=2),"/",
      ...
      for(sinkPop in 1:length(edges)){
        ...
        if(edge!="0"){
          ...
          Arrows(x0=sourcePop,x1=sinkPop,y0=add,y1=add,lwd=3,
                 arr.width=0.1,arr.adj=1,arr.type="simple",
                 col=rgb(1,1-standStrength,0,maxColorValue=1))
          if (sourcePop < sinkPop){
            text(x=sourcePop-0.5,y=-0.2+add,cex=0.8,
                 labels=paste(format(edge,scientific=TRUE,digits=2)))
          }else{
            text(x=sourcePop+0.5,y=-0.2+add,cex=0.8,
                 labels=paste(format(edge,scientific=TRUE,digits=2))) 
...
setwd("/Users/kuowenhsi/Library/CloudStorage/OneDrive-WashingtonUniversityinSt.Louis/MOBOT/MOBOT_Boltonia/data/fastsimcoal2/bistream/bestrun")
...
#######
png(paste(path,prefix,"_model.png",sep=""),height=8,width=10,units = "in", res = 600)

# plot the model with the bestlhoods parameters
# par(mfrow=c(1,1),oma=c(0,0,0,0),cex.axis=0.8,cex.main=1,cex.lab=1,cex=0.8)
