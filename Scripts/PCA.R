...
filename <- "Boltonia_Chr_1_1-23679183.filtered_subsetfilter_filtered_all_samples.pruned"
metadata <- read_csv("./Data/population_ID.csv")
PCX <- "1"
PCY <- "2"

eigenvec <- read_table(paste0("./Data/PCA/", filename, ".eigenvec"), col_names = FALSE)
...
eigenval <- read_tsv(paste0("./Data/PCA/", filename, ".eigenval"), col_names = FALSE) |>
...
#   "scott",           -0.02,       0.004,
#   "saint clair",    -0.025,       0.005,
#   "madison",         -0.016,        0.007,
#   "jersey",           0.02,        -0.005,
#   "cass",             0.01,       0.01,
#   "morgan",          -0.02,       0.01,
#   "tazewell",         -0.025,       -0.011,
#   "peoria",          0.02,        0,
#   "frederick",       -0.02,        -0.015,
#   "schuyler",        -0.025,        -0.005,
#   "woodford",         0.02,       0.01,
#   "marshal",         0.02,        0.004,
#   "fulton1",           0.02,        -0.01,
#   "fulton2",           -0.02,        -0.01,
...
#   rowwise() |> 
#   mutate(dist = sqrt((paste0("PC", PCX) - paste0("mean_PC", PCX))^2 + (paste0("PC", PCY) - paste0("mean_PC", PCX))^2)) |> 
#   ungroup()
...
#   mutate(
#     adj_X = paste0("PC", PCX) + adj_X_shift,
#     adj_Y = paste0("PC", PCY) + adj_Y_shift
#   ) |>
#   select(-adj_X_shift, -adj_Y_shift)
# 
# #custom order based on latitude
# custom_order <- c("saint clair", "madison", "alton", "jersey", "scott", "morgan", "cass", "schuyler", "frederick", "fulton2", "fulton1", "fulton3", "peoria", "tazewell", "woodford", "marshal", "hennepin")
...
p <- ggplot() +
  ...
  #            aes(paste0("PC", PCX), paste0("PC", PCY), fill = Sample_Group, shape = Sample_Group
  #            ),
  ...
  # geom_segment(
  #   data = closest_points, 
  #   aes(x = paste0("PC", PCX), y = paste0("PC", PCY), xend = adj_X, yend = adj_Y),
  #   color = "black"
  # ) +
  # geom_label(
  #   data = closest_points,
...
p
ggplotly(p)
ggsave(paste0("./Figures/Boltonia_all_samples_PCA_", PCX, "-", PCY, ".png"), 
       width = 8, 
       height = 8, 
       dpi = 300, 
       bg = "white")
